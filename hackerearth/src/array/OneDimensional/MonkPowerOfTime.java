...
public class MonkPowerOfTime {
    ...
    public static void main(String[] args) {
        ...
        while (input.size() != 0) {
            job = input.poll();
            executed_job = order.peek();
            System.out.println("process to be executed is " + executed_job + " process available " + job);
            if (job != executed_job) {
                System.out.println("process being delayed");
                input.add(job);
                total_time++;
            } else {
            ...
            System.out.println("Process Que is " + input);
        }
